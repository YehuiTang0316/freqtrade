...
class CCIPulseV3(IStrategy):
    ...
    # v2 JSON params as defaults (no JSON override needed)
    minimal_roi = {"0": 0.609, "305": 0.193, "569": 0.079, "1729": 0}
    stoploss = -0.07  # tightened from -0.094
    trailing_stop = True
    ...
    cci_bull = IntParameter(80, 150, default=109, space="buy", optimize=True)
    cci_bear = IntParameter(-150, -80, default=-87, space="sell", optimize=True)
    ...
    sell_adx_min = IntParameter(10, 25, default=11, space="sell", optimize=True)
    buy_volume_mult = DecimalParameter(1.0, 3.0, default=1.5, decimals=1, space="buy", optimize=True)
    sell_volume_mult = DecimalParameter(0.5, 2.0, default=0.6, decimals=1, space="sell", optimize=True)
    ...
    def custom_exit(self, pair, trade, current_time, current_rate, current_profit, **kwargs):
        ...
        # 4h trend flip exit — cut losing positions, let winners ride via trailing stop
        if current_profit < 0:
            if not trade.is_short and last.get("trend_4h", 0) == -1:
                return "long_exit_trend_flip"
            if trade.is_short and last.get("trend_4h", 0) == 1:
