...
class StochMomPulse(IStrategy):
    """
    StochMomPulse v1: Stochastic RSI momentum + squeeze detection on 1h.

    Combines StochRSI momentum crossover with BB/Keltner squeeze for timing.
    StochRSI catches momentum shifts faster than regular RSI.
    Entry when StochRSI crosses out of extreme zone while squeeze is releasing.
    """
    ...
    buy_stoch_k = IntParameter(10, 30, default=20, space="buy", optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        stoch_cross_up = (
            (dataframe["stoch_k"] > self.buy_stoch_k.value)
            & (dataframe["stoch_k_prev"] <= self.buy_stoch_k.value)
            & (dataframe["stoch_k"] > dataframe["stoch_d"])
        )

        # StochRSI crossing down from overbought
        stoch_cross_down = (
            (dataframe["stoch_k"] < self.sell_stoch_k.value)
            & (dataframe["stoch_k_prev"] >= self.sell_stoch_k.value)
            & (dataframe["stoch_k"] < dataframe["stoch_d"])
        )

        # Long: squeeze release + StochRSI momentum up
        long_1 = (
            btc_ok_long
        ...
        # Long 2: Momentum breakout with StochRSI confirmation
        long_2 = (
            btc_ok_long
        ...
        dataframe.loc[long_1, ["enter_long", "enter_tag"]] = (1, "long_squeeze_stoch")
        dataframe.loc[long_2 & ~long_1, ["enter_long", "enter_tag"]] = (1, "long_momentum_stoch")

        # Short: squeeze release + StochRSI momentum down
        short_1 = (
            btc_ok_short
            & (dataframe["trend_4h"] <= 0)
            & (prev_squeeze | dataframe["bb_tight"])
        ...
        dataframe.loc[short_1, ["enter_short", "enter_tag"]] = (1, "short_squeeze_stoch")
        dataframe.loc[short_2 & ~short_1, ["enter_short", "enter_tag"]] = (1, "short_momentum_stoch")
